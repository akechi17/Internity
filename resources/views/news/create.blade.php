...
@section('dashboard-content')
    <x-form.form formTitle="Tambah Berita/Artikel" formMethod="POST" formAction="{{ route('news.store') }}" enctype="multipart/form-data">
        <x-slot:formBody>
            ...
            <input type="text" name="newsable_id" value="{{ $category == 'school' ? $selectedSchool->id : $selectedDepartment->id }}">
            <input type="text" name="newsable_type" value="{{ $category == 'school' ? 'App\Models\School' : 'App\Models\Department' }}">
        </x-slot:formBody>
    </x-form.form>
@endsection
